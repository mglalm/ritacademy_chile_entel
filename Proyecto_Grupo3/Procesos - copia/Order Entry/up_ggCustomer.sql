-- Ericsson Software Inc. 2001-2012

-- DATABASE UPGRADE SCRIPT OF data_order. PRODUCT NO 00016000
-- DATABASE UPGRADE SCRIPT OF wizard. PRODUCT NO 00005000
-- DATABASE UPGRADE SCRIPT OF customer. PRODUCT NO 00005000
-- DATABASE UPGRADE SCRIPT OF party. PRODUCT NO 00005000
-- DATABASE UPGRADE SCRIPT OF cwl_customer. PRODUCT NO 20130101
-- DATABASE UPGRADE SCRIPT OF SIDCommon. PRODUCT NO 00005201
-- DATABASE UPGRADE SCRIPT OF ecmDTLib. PRODUCT NO 20140328
-- DATABASE UPGRADE SCRIPT OF ecmCS6import. PRODUCT NO 00000001
-- DATABASE UPGRADE SCRIPT OF customer_information_management. PRODUCT NO 00005000
-- DATABASE UPGRADE SCRIPT OF serviceOrchestrationFramework. PRODUCT NO 20120815
-- DATABASE UPGRADE SCRIPT OF ecmBillingAdapterValidation FROM PRODUCT NO 00000001 TO PRODUCT NO 1
-- DATABASE UPGRADE SCRIPT OF cwInitialization. PRODUCT NO 00005000
-- DATABASE UPGRADE SCRIPT OF cwl_ims. PRODUCT NO 20130319
-- DATABASE UPGRADE SCRIPT OF ecm. PRODUCT NO 20140910
-- DATABASE UPGRADE SCRIPT OF order_negotiations. PRODUCT NO 00005000
-- DATABASE UPGRADE SCRIPT OF order_negotiations_lib. PRODUCT NO 00005000
-- DATABASE UPGRADE SCRIPT OF service_registry. PRODUCT NO 00006000
-- DATABASE UPGRADE SCRIPT OF catalog. PRODUCT NO 00006018
-- DATABASE UPGRADE SCRIPT OF api_common. PRODUCT NO 20130410
-- DATABASE UPGRADE SCRIPT OF cwl_party. PRODUCT NO 00005000
-- DATABASE UPGRADE SCRIPT OF cwl_security. PRODUCT NO 20120817
-- DATABASE UPGRADE SCRIPT OF cwl_service_registry. PRODUCT NO 00006003
-- DATABASE UPGRADE SCRIPT OF cwl_address. PRODUCT NO 20120910
-- DATABASE UPGRADE SCRIPT OF plugin_on_cim. PRODUCT NO 00005000
-- DATABASE UPGRADE SCRIPT OF address. PRODUCT NO 00005000
-- DATABASE UPGRADE SCRIPT OF cwl_config. PRODUCT NO 20131210
-- DATABASE UPGRADE SCRIPT OF cwt_common5. PRODUCT NO 20130315
-- DATABASE UPGRADE SCRIPT OF data_dictionary. PRODUCT NO 00005000
-- DATABASE UPGRADE SCRIPT OF catalogClient. PRODUCT NO 00006003


-- ATTENTION: THIS SCRIPT HAS TO BE EXAMINED BY DATABASE ADMINISTRATOR!
--            IT MAY NOT CONTAIN ALL NEEDED PARAMETERS.
--            MODIFY THE SCRIPT TO SPECIFY PROPER TABLESPACE AND STORAGE CLAUSES

-- Generated by Ericsson Service Designer on Wed Mar 01 10:05:59 CLT 2017


-- THE FOLLOWING STATEMENTS APPLY TO 'C##TRAIN_2:orcl@localhost' DATABASE ------------------------------- 


define DEFAULT_TABLESPACE_TABLE = USERS;
define DEFAULT_TABLESPACE_INDEX = USERS;
define PE_TABLESPACE_TABLE = USERS;
define PE_TABLESPACE_INDEX = USERS;

CREATE TABLE C##TRAIN_2.gg_customerProfile(
	CWDOCID VARCHAR2(16) NOT NULL,
	CUSTOMERRUT VARCHAR2(10),
	CUSTOMERMSISDN NUMBER(9),
	CUSTOMERNAME VARCHAR2(24),
	CUSTOMERTYPE VARCHAR2(1),
	CUSTOMERADDRESS VARCHAR2(24),
	CUSTOMEREMAIL VARCHAR2(24),
	CWCREATEDBY VARCHAR2(64),
	CWORDERCREATIONDATE DATE,
	CONSTRAINT PK_gg_customerProfile PRIMARY KEY(CWDOCID) USING INDEX
	(CREATE UNIQUE INDEX PK_gg_customerProfile ON gg_customerProfile(CWDOCID) TABLESPACE &DEFAULT_TABLESPACE_INDEX)
)
TABLESPACE &DEFAULT_TABLESPACE_TABLE;




COMMIT;


-- Generated by Ericsson Service Designer on Wed Mar 01 10:05:59 CLT 2017

